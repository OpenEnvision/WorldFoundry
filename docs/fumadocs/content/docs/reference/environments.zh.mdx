...
| `cosmos3` | `worldfoundry-cosmos3-cu128` | `bash scripts/inference/prepare_model_infer.sh cosmos3 --download` | Cosmos3 需要 torch 2.10、torchvision 0.25、Diffusers 0.39 和 Cosmos Guardrail。精确版本的 Cosmos3 pipeline/transformer/AVAE 推理增量已在仓内；setup 只安装发布包，不 clone 源码仓库。 |
