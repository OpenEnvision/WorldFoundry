...
User-facing CLI, TUI, and Workspace flows enter through `execution/orchestration/service.py`. That module validates one of five intent dataclasses and compiles it to either `ModelBenchmarkRunRequest` or `EvaluateRunRequest`. Frontends must not duplicate generation, artifact materialization, benchmark dispatch, or metric execution. New UX should normally add an intent-to-request mapping, not another runner.

Keep the three planning boundaries distinct. `PreparedEvaluation` is the user-facing compiler result and owns preflight diagnostics plus protocol-fidelity policy. `RunPlan` is the lower-level plan for one generation/evaluate stage. `ModelBenchmarkRunRequest` is the two-stage generation-plus-official-evaluator request. Do not wrap these in a fourth recipe executor; reproduction profiles and UI intents must compile onto these existing request types.
