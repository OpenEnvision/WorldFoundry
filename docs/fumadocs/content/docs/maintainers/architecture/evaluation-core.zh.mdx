...
面向用户的 CLI、TUI 与 Workspace 都从 `execution/orchestration/service.py` 进入。该模块校验五种 intent，并编译为已有的 `ModelBenchmarkRunRequest` 或 `EvaluateRunRequest`；前端不能重复实现 generation、artifact materialization、benchmark dispatch 或 metric execution。

三层 planning 边界必须保持清晰：`PreparedEvaluation` 是面向用户的编译结果，负责 preflight diagnostics 与 protocol-fidelity policy；`RunPlan` 是单个 generation/evaluate stage 的低层计划；`ModelBenchmarkRunRequest` 是 generation 加 official evaluator 的两阶段请求。不要再包一层新的 recipe executor，reproduction profile 和 UI intent 都应编译到这些已有 request type。
